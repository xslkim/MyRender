...
//    }
    ...
    half3 SampleProbeSHVertex(float3 absolutePositionWS, float3 normalWS, float3 viewDir)
    {
#if (defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2))
...
//        bool probe0Dominant = importanceSign > 0.0f || (importanceSign == 0.0f && volumeDiff < -0.0001f);
//        bool probe1Dominant = importanceSign < 0.0f || (importanceSign == 0.0f && volumeDiff > 0.0001f);
//
...
real PositivePow(real base, real power)
{
    return Pow(max(abs(base), (real)FLT_EPSILON), power);
}
...
half4 SAMPLE_TEXTURECUBE_LOD(Texture2D* cube_tex, SamplerState state, real3 reflect_vector, real mip)
{
        ...
        half3 normalWS, half3 viewDirectionWS, float2 normalizedScreenSpaceUV)
    {
        ...
        half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, positionWS, brdfData.perceptualRoughness, 1.0f, normalizedScreenSpaceUV);
    ...
    void MixRealtimeAndBakedGI(Light& light, half3 normalWS, half3& bakedGI)
    {
    ...
    void MixRealtimeAndBakedGI(Light& light, half3 normalWS, half3& bakedGI, half4 shadowMask)
    {
        MixRealtimeAndBakedGI(light, normalWS, bakedGI);
    }

    void MixRealtimeAndBakedGI(Light& light, half3 normalWS, half3& bakedGI, AmbientOcclusionFactor aoFactor)
    {

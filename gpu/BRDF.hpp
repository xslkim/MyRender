...
#define kDielectricSpec half4(0.04, 0.04, 0.04, 1.0 - 0.04) // standard dielectric reflectivity coef at incident angle (= 4%)
    ...
    inline void InitializeBRDFDataDirect(half3 albedo, half3 diffuse, half3 specular, half reflectivity, half oneMinusReflectivity, half smoothness, half& alpha, BRDFData& outBRDFData)
    {
    ...
    inline void InitializeBRDFDataDirect(half3 diffuse, half3 specular, half reflectivity, half oneMinusReflectivity, half smoothness, half& alpha, BRDFData& outBRDFData)
    {
    ...
    inline void InitializeBRDFData(half3 albedo, half metallic, half3 specular, half smoothness, half& alpha, BRDFData& outBRDFData)
    {
    ...
    inline void InitializeBRDFData(SurfaceData& surfaceData, BRDFData& brdfData)
    {
    ...
    {
        half3 out;
        out.x = saturate(-0.0256868f + f0.x * (0.326846f + (0.978946f - 0.283835f * f0.x) * f0.x));
        out.x = saturate(-0.0256868f + f0.y * (0.326846f + (0.978946f - 0.283835f * f0.y) * f0.y));
        out.x = saturate(-0.0256868f + f0.z * (0.326846f + (0.978946f - 0.283835f * f0.z) * f0.z));
        return out;
    }

    inline void InitializeBRDFDataClearCoat(half clearCoatMask, half clearCoatSmoothness, BRDFData& baseBRDFData, BRDFData& outBRDFData)
    {
        //outBRDFData = (BRDFData)0;
        outBRDFData.albedo = half3(1,1,1);
        ...
        half ieta = lerp(1.0f, CLEAR_COAT_IETA, clearCoatMask);
        ...
        baseBRDFData.normalizationTerm = baseBRDFData.roughness * 4.0f + 2.0f;
        baseBRDFData.roughness2MinusOne = baseBRDFData.roughness2 - 1.0f;
    ...
    BRDFData CreateClearCoatBRDFData(SurfaceData surfaceData, BRDFData& brdfData)
    {
    ...
    half3 EnvironmentBRDFSpecular(BRDFData brdfData, half fresnelTerm)
    {
        ...
        float3 grazingTerm(brdfData.grazingTerm, brdfData.grazingTerm, brdfData.grazingTerm);
        return half3(surfaceReduction * lerp(brdfData.specular, grazingTerm, fresnelTerm));
    }
    ...
    half DirectBRDFSpecular(BRDFData brdfData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS)
    {
        ...
        half LoH2 = LoH * LoH;
        half specularTerm = brdfData.roughness2 / ((d * d) * max(0.1f, LoH2) * brdfData.normalizationTerm);

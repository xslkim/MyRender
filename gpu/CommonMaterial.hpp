...
//-----------------------------------------------------------------------------
    ...
    void ConvertValueAnisotropyToValueTB(real value, real anisotropy, real& valueT, real& valueB)
    {
    ...
    void ConvertAnisotropyToRoughness(real perceptualRoughness, real anisotropy, real& roughnessT, real& roughnessB)
    {
    ...
    void ConvertRoughnessTAndAnisotropyToRoughness(real roughnessT, real anisotropy, real& roughness)
    {
    ...
    void ConvertRoughnessToAnisotropy(real roughnessT, real roughnessB, real& anisotropy)
    {
    ...
    void ConvertAnisotropyToClampRoughness(real perceptualRoughness, real anisotropy, real& roughnessT, real& roughnessB)
    {
...
#define NORMALMAP_HIGHEST_VARIANCE 0.03125
    ...
    real3 BlendNormalWorldspaceRNM(real3 n1, real3 n2, real3 vtxNormal)
    {
        ...
        real4 q = real4(cross(vtxNormal, n2), dot(vtxNormal, n2) + 1.0f) / sqrt(2.0f * (dot(vtxNormal, n2) + 1));

        // Rotate the normal
        return n1 * (q.w * q.w - dot(q.xyz, q.xyz)) + 2.f * q.xyz * dot(q.xyz, n1) + 2.f * q.w * cross(q.xyz, n1);
    }
    ...
    real3 ComputeTriplanarWeights(real3 normal)
    {
        ...
        blendWeights /= dot(blendWeights, real3(1,1,1));

        return blendWeights;
    }

    // Planar/Triplanar convention for Unity in world space
    void GetTriplanarCoordinate(float3 position, float2& uvXZ, float2& uvXY, float2& uvZY)
    {

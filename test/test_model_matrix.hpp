...
{
	std::ifstream f("assets/model_test.json");
	if (f.is_open())
	{
		json j = json::parse(f);
		for (auto& item : j) {
			...
			float scale_x = item["scale_x"].template get<float>();
			float scale_y = item["scale_y"].template get<float>();
			float scale_z = item["scale_z"].template get<float>();
